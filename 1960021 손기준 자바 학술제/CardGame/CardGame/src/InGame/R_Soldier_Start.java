...
import Character.*;
/*
 플레이어가 경찰을 선택했을시 게임 메소드를 담고 있는 클래스 
 */
public class R_Soldier_Start {
	...
	// 플레이어
		...
		public static void R_Start() { // 퇴역군인을 선택 할 시 시작
			Retired_Soldier Player = new Retired_Soldier(); //플레이어 객체 생성
			...
			if(computer == 1) {
				...
				InGame_Print.Selected_R(); // 선택된 화면 출력 함수
						...
						while(true){
							...
							if(Player.Selected_Unique == true && Com.Selected_Unique == true) {
								...
								if(Player.Success_Unique == true) {
									P_Life ++;   // 유니트 성공시 유니크 데미지 만큼 상대 체력 감소
								}
				...
				try {
					...
					System.out.println("\n\n\n\n\n\n\n");
					System.out.println("\t\t\t당신(퇴역군인)은 이 결투에서 승리했습니다!!!\n");
					System.out.println("\n\n\n\n\n\n\n\n");
					}
					else if(P_Life <= C_Life) {
						System.out.println("\n\n\n\n\n\n\n");
						System.out.println("\t\t\t당신(퇴역군인)은 이 결투에서 패배 했습니다!!!\n");
						System.out.println("\n\n\n\n\n\n\n\n");
						}
							...
							if(P_Select_Card == 1) {
								...
								if(Player.Hit_Pistol == true) {
									if(Com.Success_Cover == true || Com.Success_Unique == true) {
										...
										System.out.println("명중 !! 상대에게 "+Com.getP_Damage()+"데미지를 입혔습니다.");
										}
								}
								else if(Player.Hit_Gun == false) {
									System.out.println("공격이 빗나갔습니다.");
