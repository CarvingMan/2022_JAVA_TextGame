...
public class Police_Start {
	...
	// 플레이어
		...
		public static void P_Start() { // 경찰을 선택 할 시 시작
			Police Player = new Police(); //플레이어 객체 생성
			P_Life = Player.getLife();  // 경찰 체력 저장
			...
			if(computer == 1) {
				...
				InGame_Print.Selected_P(); // 선택된 화면 출력 함수
				...
				try {
					...
					System.out.println("\n\n\n\n\n\n\n");
					System.out.println("\t\t\t당신(경찰)은 이 결투에서 승리했습니다!!!\n");
					System.out.println("\n\n\n\n\n\n\n\n");
					}
					else if(P_Life <= C_Life) {
						System.out.println("\n\n\n\n\n\n\n");
						System.out.println("\t\t\t당신(경찰)은 이 결투에서 패배 했습니다!!!\n");
						System.out.println("\n\n\n\n\n\n\n\n");
						}
				...
				InGame_Print.Selected_P(); // 선택된 화면 출력 함수
				for(int i = 1; i <= turn; i++) {
					if((Com.getLife() <=0 )|| (Player.getLife() <= 0)) {   // 체력이 0보다 작거나 같을 시 바로 게임종료
						break;
					}
					else {
						while(true){
